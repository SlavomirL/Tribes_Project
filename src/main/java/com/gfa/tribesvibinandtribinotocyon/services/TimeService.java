...
public class TimeService {
    ...
    public Date build(Long userId, String buildingName) throws TribesInternalServerErrorException {
        ...
        if (buildingInfo != null) {
            ...
            Runnable builder = () -> buildingService.actualUpgradeBuilding(userId, buildingName);
    ...
    public Date recruit(Long userId, String troopName, Integer troopAmount) {
        ...
        Runnable recruiter = () -> troopService.actualTroopRecruit(userId, troopName, troopAmount);
    ...
    public Date move(Long userId, Integer baseXCoordinates, Integer baseYCoordinates, Integer targetXCoordinates, Integer targetYCoordinates, List<Troop> troops) {
        ...
        Integer slowestUnit = troops.stream()
                ...
                .orElse(1);
        Long marchingDistance = (long) Math.pow(Math.pow((baseXCoordinates - targetXCoordinates), 2) + (Math.pow((baseYCoordinates - targetYCoordinates), 2)), 0.5);
        Long marchingTime = marchingDistance / slowestUnit;
        Runnable mover = () -> troopService.actualTroopMovement(userId, troops, targetXCoordinates, targetYCoordinates);
        ScheduledFuture<?> movementHandle = scheduler.schedule(mover, marchingTime, TimeUnit.HOURS);

        Date currentDate = new Date();
        long marchingTimeDate = MILLISECONDS_IN_SECOND * SECONDS_IN_MINUTE * MINUTES_IN_HOUR * marchingTime;
        return new Date(currentDate.getTime() + marchingTimeDate);
    ...
    public Date hourlyResourceGathering() {
        Runnable gatherer = () -> {
            try {
                kingdomService.calculateHourlyGatheringForAllKingdoms();
            } catch (TribesNotFoundException e) {
                e.printStackTrace();
            }
        };
